...
%for m=[22,23];
    ...
    % aexp
    for k=1:length(vars);
        ncfile=[datapath,'/',igrp{m},'/',imod{m},'/' exps{n} '_' ares '/', vars{k}, '_2100' '_GIS_', igrp{m},'_',imod{m},'_', exps{n}, '.nc'];
        % check file exists
        if exist(ncfile, 'file') == 2
            ncfile
            in=ncload(ncfile);
            eval(['li.' exps{n} '{m}.', vars{k},'=single(in.',vars{k},'(:,:));']);
        else
            %eval(['li.' exps{n} '{m}.', vars{k},'=single(zeros(337,577));']);
            eval(['li.' exps{n} '{m}.', vars{k},'=[];']);
        end
    end
