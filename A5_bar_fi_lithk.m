...
for m=1:(diag.n-4)
    ...
    %bar(m,diag.rms_lithk{k},'FaceColor',colors(m,:));
    bar(m,diag.rms_lithk_ssm{k},'FaceColor',colors(m,:));
    % mark up init method
    %if(ch.dasp(k)==1)
...
% save
%print('-r300', '-dpng', ['Figures/' 'A5_bar_fi_diag_lithk']);
print('-r300', '-dpng', ['Figures/' 'A5_bar_fi_diag_lithk_ssm']);
